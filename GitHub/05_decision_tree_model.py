...
def save_results(model, train_report, test_report, importance_df):
    ...
    model_path = os.path.join(OUTPUT_MODEL_DIR, 'tree_TB_color.pkl')
    with open(model_path, 'wb') as f:
        pickle.dump(model, f)
    print(f"\nTrained Decision Tree model saved to {model_path}")

    train_report_path = os.path.join(OUTPUT_REPORT_DIR, 'TB_tree_train_report.csv')
    test_report_path = os.path.join(OUTPUT_REPORT_DIR, 'TB_tree_test_report.csv')
    importance_path = os.path.join(OUTPUT_REPORT_DIR, 'TB_tree_importance.csv')
...
if __name__ == '__main__':
    print("Starting Decision Tree model training and evaluation")
    random.seed(RANDOM_SEED)
    np.random.seed(RANDOM_SEED)

    category_map_with_ext = [[item[0] + '.png', item[1]] for item in CATEGORY_MAP_LIST]

    X_train, X_test, y_train, y_test, _ = load_and_preprocess_data(INPUT_CSV, category_map_with_ext,
                                                                    TRAINSET_FILENAMES, TESTSET_FILENAMES)
    trained_model, train_rep, test_rep, importance_df = train_and_evaluate_decision_tree(X_train, X_test, y_train, y_test)
