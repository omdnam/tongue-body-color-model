...
def save_results(model, train_report, test_report, importance_df):
    ...
    model_path = os.path.join(OUTPUT_MODEL_DIR, 'forest_TB_color.pkl')
    with open(model_path, 'wb') as f:
        pickle.dump(model, f)
    print(f"\nTrained Random Forest model saved to {model_path}")

    train_report_path = os.path.join(OUTPUT_REPORT_DIR, 'TB_forest_train_report.csv')
    test_report_path = os.path.join(OUTPUT_REPORT_DIR, 'TB_forest_test_report.csv')
    importance_path = os.path.join(OUTPUT_REPORT_DIR, 'TB_forest_importance.csv')
...
if __name__ == '__main__':
    ...
    trained_model, train_rep, test_rep, importance_df = train_and_evaluate_random_forest(X_train, X_test, y_train, y_test)
